...
let ksUrl = [], ksArr = [];
  ...
  if (process.env.KUAISHOUMV_SAM && process.env.KUAISHOUMV_SAM.indexOf('#') > -1) {
  ksUrl = process.env.KUAISHOUMV_SAM.split('#');
  console.log(`您选择的是用"#"隔开\n`)
  }
  else if (process.env.KUAISHOUMV_SAM && process.env.KUAISHOUMV_SAM.indexOf('\n') > -1) {
  ksUrl = process.env.KUAISHOUMV_SAM.split('\n');
...
!(async () => {
 ...
 //   }
   ...
   console.log(`-------------------------\n\n快手视频共完成任务${$.index}次，视频金币详情见App，任务全部结束`)
})()
  .catch((e) => $.logErr(e))
  .finally(() => $.done())
 ...
 return new Promise((resolve) => { 
   const myRequest = {
    ...
    headers: {
      ...
      'User-Agent' : `com_kwai_nebula/3.2.10 (iPhone; iOS 14.2; Scale/3.00)`,

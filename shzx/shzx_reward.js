...
let Account = ["【Sam】"];
  ...
  if (process.env.SHZX_BD && process.env.SHZX_BD.indexOf('\n') > -1) {
  BD = process.env.SHZX_BD.split('\n');
  console.log(`您选择的是用换行隔开\n`)
  } 
  else
  {
  ...
  if (process.env.SHZX_BD_AD && process.env.SHZX_BD_AD.indexOf('\n') > -1) {
  ADBD = process.env.SHZX_BD_AD.split('\n');
  console.log(`您选择的是用换行隔开\n`)
  } 
  else
  {
  ...
  Object.keys(ADBD).forEach((item) => {
        if (ADBD[item]) {
          ADBDArr.push(ADBD[item])
        }
  })
